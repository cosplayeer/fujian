...
def plotobs():
    ...
    plotcols = ['spd70','ECWindSpeed10','ECWindSpeed50','ECWindSpeed150','GFSWindSpeed10']
    df_plot = df_all[plotcols]
    df_plot['ECWindSpeed50'] = df_plot['ECWindSpeed50'] * 0.8
    df_plot['ECWindSpeed150'] = df_plot['ECWindSpeed150'] * 0.7
    print("HIIIIIIIIIIIIIIIIII")
    print(df_plot['ECWindSpeed50'].min()) # -0.8
    print(df_plot['ECWindSpeed150'].min()) # -1.7
    ...
    plt.savefig("./plot/month456/data456.png")
    plt.clf()
plotobs()
...
def SelectFeatures():
    ...
    predictors2 = ['ECWindSpeed10','GFSWindSpeed10',
                    'ECWindSpeed10_1','GFSWindSpeed10_1',
                    'ECWindSpeed10_2','GFSWindSpeed10_2']

    df2 = df_all[['spd70'] + predictors2]
    df2.columns = ['spd70','ECWindSpeed10','GFSWindSpeed10',
                    'ECWindSpeed10_1','GFSWindSpeed10_1',
                    'ECWindSpeed10_2','GFSWindSpeed10_2']
    # print(df2)
    print(df2.info())
...
def plotcorrpanel():
    ...
    arr = np.array(predictors).reshape(2, 3)
    ...
    #plt.show()
    plt.savefig("plot/month456/corrpanel_2.png")
    plt.clf()
    #-------------step 0 plot end----------------------#
plotcorrpanel()
...
def plotscatter():
    ...
    plt.savefig("./plot/month456/scatter.png")
    plt.clf()
# plotscatter()
...
def plothot1():
    ...
    plt.savefig("plot/month456/hotfig.png")
    plt.clf()
...
def plothot2():
    ...
    print("plot/month456/hotfig2.png　ploted")
    for i in range(6):
        for j in range(6):
            if corr[i,j] < 0.9999:
    ...
    plt.savefig("plot/month456/hotfig3.png")
    plt.clf()
plothot2()

def BuildModel():
    #Using Step-wise Regression to Build a Robust Model
    X = df2[predictors][:1417] # 20190101-20190228
    y = df2['spd70'][:1417]
...
def train_regress():
    ...
    # print(prediction)
    prediction_origin_one = df2['ECWindSpeed10'][1417:]
    # print(df2)
    ...
    result0.to_csv("./data/month456/output_obs_prediction_456month_origin_one.csv",float_format="%.2f")
    # no origin 订正后的3月份预报
    result=pd.concat([y_test,pd.DataFrame({'end_var':prediction},index=y_test.index)],axis=1).dropna()
    result.to_csv("./data/month456/output_obs_prediction_456month.csv",float_format="%.2f")
